...
def main():
    ...
    code = input("Code (gaussian/orca/qchem/psi4/nwchem/gamess) : ").lower()
    ...
    orbitals = input(
        "Orbitales à étendre (ex: S,P,D ou ENTER pour aucune) : "
    ).upper()

    if orbitals.strip() == "":
        print("Aucune extension demandée.")
        sys.exit()
    ...
    new_basis = inject_primitives(basis_text, extensions, code)
